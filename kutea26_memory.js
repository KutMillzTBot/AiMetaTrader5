...
Mode: IDLE</div>
  ...
  <script>
    ...
    function renderLive(rows = []) {
      ...
      el.liveBody.innerHTML = html || `<tr><td colspan="9" class="hint">No live transactions</td></tr>`;
    }
    ...
    function renderHistory(rows = []) {
      ...
      el.historyBody.innerHTML = html || `<tr><td colspan="9" class="hint">No completed trades</td></tr>`;
    }
    ...
    function openTradeDetailModal(row, source = "live") {
      ...
      const details = [
        ...
        ["Source", source === "history" ? "History" : "Live"],
    ...
    function drawLivePositionOverlays(ctx, vp, digits) {
      ...
      rows.forEach((t, idx) => {
        ...
        if (Number.isFinite(slPx) && slPx > 0) {
          const active = !!(draft && Number(draft.ticket) === Number(t.id || 0) && draft.level === "sl");
        ...
        if (Number.isFinite(tpPx) && tpPx > 0) {
          const active = !!(draft && Number(draft.ticket) === Number(t.id || 0) && draft.level === "tp");
